...
DemoApp = function() {
   ...
   var jumpIncrement = .005;     //time between randomly reassigning curve targets
	...
	this.setup = function() {
      ...
      for(var i=0; i< numQuads; i++){
         makeQuad( triangle_geom , LAB.random( .051, .949 ));
      }
      ...
      for(var i=0; i<triangle_geom.vertices.length; i = i+4){
         var colIndex = Math.min(3, LAB.randomInt( 0, 4 ));//random color pair
      ...
      for(var p = 0; p < particleCount; p++) {
         var goal = LAB.randomInt( 0, pOnCnodes.length-1);
         pGoals.push( goal );
         pVels.push( new THREE.Vector3( LAB.random(-1, 1), LAB.random(-1, 1), LAB.random(-1, 1) ));
      ...
      for( var v = 0; v < particles.vertices.length; v++ ) {
         attributes.xOffset.value[ v ] = LAB.randomInt(0, 5)/5;
         attributes.yOffset.value[ v ] = LAB.randomInt(0, 8)/8;
      }
      ...
      for(var i=0; i<numLines; i++){
         var meshIndex = Math.min( LAB.randomInt( 0, 8 ), 7);//one mesh per color in pallette
         
         var randIndex = LAB.randomInt(0, particles.vertices.length-1 );
         ...
         if(i % 2){
            line_geom[meshIndex].vertices.push( new THREE.Vertex( vec1 ));
            line_geom[meshIndex].vertices.push( new THREE.Vertex(LAB.randomObject( pOnCnodes ).position));
         }
      }
   ...
   this.update = function (){
      ...
      for(var i=0; i<curves.length; i++){
         for(var j=1; j<curves[i].points.length; j++){
            xScl = Math.cos( LAB.map( curves[i].points[j].x, 0, window.innerWidth, 0, 3.14 ));
      ...
      for(var i=pOnCnodes.length-1; i>=0; i--){
         ...
         if(pOnCnodes[i].uVal > 1){
            ...
            pOnCnodes[i-3].curve = pOnCnodes[i].curve = curves[ LAB.randomInt( 0, curves.length) ];
            pOnCnodes[i-1].curve = pOnCnodes[i-2].curve = curves[ LAB.randomInt( 0, curves.length) ];
         }
         evalPointOnCurve( pOnCnodes[i] );
      }
      ...
      if(jumpTime <elapsedTime){
         ...
         var randIndex = LAB.randomInt( 0, lines_particleIndices.length-1 );
         var part = lines_particleIndices[ randIndex ];
         var vert = lines_verts[ randIndex ];
         
         pGoals[ part ] = LAB.randomInt( 0, pOnCnodes.length-1 );
         vert.position = pOnCnodes[ pGoals[ part ] ].position;
         
         //      var randPonC = LAB.randomObject( pOnCnodes );
         //      randPonC.curve = LAB.randomObject( curves );
      }
   ...
   function makeQuad( _geom, uCenter ){
      ...
      pOnC[0].curve = pOnC[3].curve = curves[ LAB.randomInt( 0, curves.length) ];
      pOnC[1].curve = pOnC[2].curve = curves[ LAB.randomInt( 0, curves.length) ];
      
      pOnC[0].uVal = LAB.random( -.05, -.015) + uCenter;
      pOnC[3].uVal = LAB.random( .015, .05) + uCenter;
      var lowmidCV = LAB.random(.25, .4);
      pOnC[1].uVal = LAB.map( lowmidCV, 0, 1, pOnC[0].uVal, pOnC[3].uVal);
      pOnC[2].uVal = LAB.map( lowmidCV + LAB.random(.25, .4), 0, 1, pOnC[0].uVal, pOnC[3].uVal);
...
//   function LAB.randomObject( _array ){
