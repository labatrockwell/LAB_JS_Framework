...
if(!window['ApplicationMessageHandler']){
	function ApplicationMessageHandler (uuid){
		...
		this.connect = function (inport, outport)
		{
			...
			if (inport != -1 && outport != -1) {
	            ...
	            inSocket = new ECS_Socket();
	            outSocket = new ECS_Socket();
	    ...
	    *********************************************************/       
	    ...
	    this._init       =   function blank_init(){
		};
	
	    //return Sage.extend(this, 'EventDispatcher');
	}
}
